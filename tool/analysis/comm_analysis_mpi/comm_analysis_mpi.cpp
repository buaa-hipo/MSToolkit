...
// add rank_start and rank_end for different analysis ranks?
CommunicationAnalyzer::CommunicationAnalyzer(RecordReader& reader, RecordTraceCollection& collection, BacktraceCollection *backtraces, RankMetaCollection& metas, int topk, double threshold_lwt, double threshold_wtv, double threshold_S, double threshold_n, double threshold_t, bool pretty_print, const char *filename) 
	...
	int rank_start = (n / nrank) * myrank;
	int rank_end = (myrank == (nrank - 1)) ? n : (rank_start + (n / nrank));
	...
	std::cout << "nrank = " << nrank << ", nthreads = " << nThreads << ", rank_start = " << rank_start << ", rank_end = " << rank_end << std::endl;
    }
	}
    ...
    f_out5.open(output_dir + std::string("/result.") + std::to_string(rank_start) + std::string("-") + std::to_string(rank_end) + std::string(".comm_mtx"));
	f_out5 << n << std::endl;
	...
	dump_report(filename, rank_start, rank_end);
}
...
void CommunicationAnalyzer::dump_report(const char *filename, int rank_start, int rank_end) {
    std::ofstream f_out1, f_out2, f_out3, f_out4;
    std::string output_dir(filename);
	
    f_out1.open(output_dir + std::string("/result.") + std::to_string(rank_start) + std::string("-") + std::to_string(rank_end) + std::string(".long_wait"));
    f_out2.open(output_dir + std::string("/result.") + std::to_string(rank_start) + std::string("-") + std::to_string(rank_end) + std::string(".high_prop"));
    f_out3.open(output_dir + std::string("/result.") + std::to_string(rank_start) + std::string("-") + std::to_string(rank_end) + std::string(".high_vol"));
    f_out4.open(output_dir + std::string("/result.") + std::to_string(rank_start) + std::string("-") + std::to_string(rank_end) + std::string(".high_freq"));
