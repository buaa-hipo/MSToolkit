...
struct STG_NVal_t {
    ...
    std::list<ParallelVarianceMap::CommVarianceRecord*> rlist;
};

struct STG_EVal_t {
    std::list<ParallelVarianceMap::CalcVarianceRecord*> rlist;
};
...
class STG {
  public:
    ...
    STG_Key_t transfer_to(int rank,
                          ...
                          int num_events, 
                          STG_Edge_t& e, 
                          ParallelVarianceMap::VarianceData* vdata) {
        std::vector<ParallelVarianceMap::CommVarianceRecord*>& commRecs = vdata->comm_vars;
        std::vector<ParallelVarianceMap::CalcVarianceRecord*>& calcRecs = vdata->calc_vars;
        ...
        if( RecordHelper::is_event(r, event_MPI_Send) ||
            RecordHelper::is_event(r, event_MPI_Recv)) {
                ParallelVarianceMap::CommVarianceRecord* cr = new ParallelVarianceMap::CommVarianceRecord({-1, rank, r});
                nval.rlist.push_back(cr);
                commRecs.push_back(cr);
        } else if(JSI_ACCL_EVENT_IS_LAUNCH(r->MsgType)) {
            // printf("## LAUNCH event ##\n");
            ParallelVarianceMap::AcclVarianceRecord* cr = new ParallelVarianceMap::AcclVarianceRecord({-1, rank, r});
            nval.rlist.push_back((ParallelVarianceMap::CommVarianceRecord*)cr);
            ...
            if ( RecordHelper::is_event(r, event_MPI_Irecv) ||
                ...
                if (it!=waiting.end()) {
                    ...
                    if (dur > overlap_wait_threshold) {
                        ...
                        ParallelVarianceMap::CommVarianceRecord* cr = new ParallelVarianceMap::CommVarianceRecord({-1, rank, (record_t*)ra});
                        nval.rlist.push_back(cr);
                        commRecs.push_back(cr);
                    }
                    waiting.erase(it);
        ...
        if (last_r!=NULL) {
            ...
            ParallelVarianceMap::CalcVarianceRecord* cr2 = new ParallelVarianceMap::CalcVarianceRecord({-1, rank, last_r, r, count});
...
bool compareIns (const ParallelVarianceMap::CalcVarianceRecord* r1, const ParallelVarianceMap::CalcVarianceRecord* r2) {
...
ParallelVarianceMap::ParallelVarianceMap(RecordTraceCollection& collection, BacktraceCollection& bt_collection, RecordTraceExtCollection& rte_collection, const std::string &ref_metric, bool enable_async_comm) {
    int initialized;
    MPI_Initialized(&initialized);
    if(!initialized) {
        JSI_ERROR("ParallelVarianceMap: MPI not initialized!");
    }
    ...
    for (int i=0; i<nkeys; ++i) {
        ...
        std::list<ParallelVarianceMap::CommVarianceRecord*>& rlist = stg.nodes[key].rlist;
        if (rlist.size()>0) {
            ...
            for(auto it=rlist.begin(), ie=rlist.end(); it!=ie; ++it) {
                ...
                if (!enc.empty() && dur) {
                    ...
                    if(si==smooth_map.end()) {
                        ...
                        // update to get minimum duration for normalization
    ...
    std::vector<ParallelVarianceMap::CalcVarianceRecord*> rlist_all;
    //for (auto ki = edge_set.begin(), ke = edge_set.end(); ki!=ke; ++ki) {
    #pragma omp for nowait
    for (int i=0; i<nedges; ++i) {
        const jsi::variance::STG_Edge_t& key = edge_vec[i];
	    {
            std::list<ParallelVarianceMap::CalcVarianceRecord*>& rlist = stg.edges[key].rlist;
    ...
    uint64_t prob_cnt_all_res[16];
    double time_all[10] = {
        tot, gpu_time, cpt, mpi_time, vpt, calc_time, pct, gct, pmt, gmt
        };
    double time_all_res[10];
    MPI_Reduce(prob_cnt_all, prob_cnt_all_res, 16, MPI_UINT64_T, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(time_all, time_all_res, 10, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    if (rank==0) {
        tot      = time_all_res[0];
        gpu_time = time_all_res[1];
        cpt      = time_all_res[2];
        mpi_time = time_all_res[3];
        vpt      = time_all_res[4];
        calc_time= time_all_res[5];
        pct      = time_all_res[6];
        gct      = time_all_res[7];
        pmt      = time_all_res[8];
        gmt      = time_all_res[9];
        ...
        printf("## [COMM       ] 100us %lu, 10ms %lu, 1s %lu, other %lu\n", prob_cnt_all_res[0], prob_cnt_all_res[1], prob_cnt_all_res[2], prob_cnt_all_res[3]);
        printf("## [CALC       ] 100us %lu, 10ms %lu, 1s %lu, other %lu\n", prob_cnt_all_res[4], prob_cnt_all_res[5], prob_cnt_all_res[6], prob_cnt_all_res[7]);
        if(enable_accl) {
            printf("## [ACCL CALC  ] 100us %lu, 10ms %lu, 1s %lu, other %lu\n", prob_cnt_all_res[8], prob_cnt_all_res[9], prob_cnt_all_res[10], prob_cnt_all_res[11]);
            printf("## [ACCL MEMCPY] 100us %lu, 10ms %lu, 1s %lu, other %lu\n", prob_cnt_all_res[12], prob_cnt_all_res[13], prob_cnt_all_res[14], prob_cnt_all_res[15]);
        }
...
ParallelVarianceMap::~ParallelVarianceMap() {
...
void ParallelVarianceMap::dump_accl_memcpy_csv(const char* filename) {
...
void ParallelVarianceMap::dump_accl_calc_csv(const char* filename) {
    if (!enable_accl) return;
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
...
void ParallelVarianceMap::dump_comm_csv(const char* filename) {
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
        JSI_ERROR("Failed to open file for write: %s\n", filename);
...
void ParallelVarianceMap::dump_calc_csv(const char* filename) {
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
        JSI_ERROR("Failed to open file for write: %s\n", filename);
...
const std::unordered_map<int, ParallelVarianceMap::VarianceData*>& ParallelVarianceMap::getVarMap() {
    return _var_map;
}


void ParallelVarianceMap::dump_comm_heatmap(const char* filename, uint64_t resolution_ms) {
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
        JSI_ERROR("Failed to open file for write: %s\n", filename);
...
void ParallelVarianceMap::dump_calc_heatmap(const char* filename, uint64_t resolution_ms) {
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
        JSI_ERROR("Failed to open file for write: %s\n", filename);
...
void ParallelVarianceMap::dump_accl_calc_heatmap(const char* filename, uint64_t resolution_ms) {
    if (!enable_accl) return ;
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
...
void ParallelVarianceMap::dump_accl_memcpy_heatmap(const char* filename, uint64_t resolution_ms) {
    if (!enable_accl) return ;
    uint64_t mhz = get_tsc_freq_mhz();
    FILE* fp = fopen(filename, "w");
    if (fp == NULL) {
