...
class ParallelVarianceMap {
  public:
    ...
    uint64_t calcVarianceRecordGetEnterTS(CalcVarianceRecord* p_cvr) { 
        return p_cvr->p_rec->timestamps.exit; 
    }

    __attribute__((always_inline))
    uint64_t calcVarianceRecordGetExitTS(CalcVarianceRecord* p_cvr) { 
        return p_cvr->c_rec->timestamps.enter; 
    }
    ...
    ParallelVarianceMap(RecordTraceCollection& collection, BacktraceCollection& bt_collection, RecordTraceExtCollection& rte_collection, const std::string& ref_metric, bool enable_async_comm);
    ~ParallelVarianceMap();
