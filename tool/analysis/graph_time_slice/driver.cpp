...
void parse(int argc, char *argv[]) {
    cxxopts::Options options("timeslice_analysis", "Read the measurement directory generated by jsirun and generate CSV files that consolidate event information based on given time slice sizes.");
    options.add_options()
    ...
    try {
        ...
        if (result.count("duration") >= 1) {
            ...
            std::cout << "-u or --duration must be specified!" << std::endl;
            exit(1);
        }
        if (result.count("interval") >= 1) {
            ...
            std::cout << "-l or --interval must be specified!" << std::endl;
            exit(1);
        }
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        DependancyGraph* graph = CreateDependancyGraphSlice(traces, metas, false, interval, duration);
        ...
        if (!dry_run) {
            if (edges_only) {
                ...
                graph->dump_nodes(output.c_str(),  pmu_event_list);
            }
        }
