...
class ChromeTraceWriter {
  ...
  public:
    ...
    void add_complete_event(std::string name, std::string cat, std::string ph, double ts, double dur, int pid, int tid, std::vector<Arg>& args) {
        writer.StartObject();
        writer.Key("name");
        writer.String(name.c_str());
        writer.Key("cat");
    ...
    void add_duration_event(std::string name, std::string cat, double ts_beg, double ts_end, int pid, int tid, std::vector<Arg>& args) {
        add_complete_event(name, cat, "X", ts_beg, ts_end-ts_beg, pid, tid, args);
    }
