...
DependancyGraph* CreateDependancyGraph(RecordTraceCollection& collection, RankMetaCollection& metas, bool need_serial_link) {
    ...
    // auto * collection_pairs = new std::pair<int, RecordTrace *>[collection.size()];

    // int i = 0;
    // for(auto it=collection.begin(), ie=collection.end(); it!=ie; ++it, ++i) {
    //     collection_pairs[i] = *it;
    // }

    std::vector<RecordTrace *> rt_list;
    std::vector<int> rank_list;
    std::vector<DependancyGraph*> graph_threads(omp_get_max_threads());
    int thread_count = omp_get_num_procs();
    for (int i = 0; i < thread_count; i++) {
        graph_threads[i] = new DependancyGraph();
    }
...
#pragma omp parallel for
    for (int i = 0; i < rank_list.size(); i++) {
        ...
        for (auto ri = rtrace.begin(), re = rtrace.end(); ri != re; ri = ri.next()) {
            ...
            graph_threads[omp_get_thread_num()]->add_node(rank, node);
            if (need_serial_link && last != NULL) {
                graph_threads[omp_get_thread_num()]->add_edge(new DependancyEdge(last, node, DependencyType::NONE));
            }
// #pragma omp critical
//             {
...
//             }
            ...
            if (RecordHelper::is_event(r, event_MPI_Wait)) {
                ...
                if (it != ltrack->wait_list.end() && it->second != NULL) {
                    ...
                    graph_threads[omp_get_thread_num()]->add_edge(new DependancyEdge(comm_node, node, DependencyType::SYNC));         
...
#pragma omp critical
    ...
    // auto * track_pairs = new std::pair<int, LastTrack_t*>[track.size()]; // array-ized for omp
    // int i = 0;
    // for(auto it=track.begin(), ie=track.end(); it!=ie; ++it, ++i) {
    //     track_pairs[i] = *it;
    // }
    std::vector<LastTrack_t*> ltrack_list;
    for (int i = 0; i < rank_list.size(); i++) {
        ltrack_list.push_back(track[rank_list[i]]);
    }
...
//    for(auto it=track.begin(), ie=track.end(); it!=ie; ++it) {
        ...
        int rank = rank_list[i];
        LastTrack_t* ltrack = ltrack_list[i];
                ...
                if(ttrack!=track.end()) {
                    ...
                    if(trq!=ttrack->second->recv_list.end()) {
                        ...
                        if (rq.empty()) {
                            ...
                            graph_threads[omp_get_thread_num()]->add_edge(new DependancyEdge(sn, rn, DependencyType::COMM));
                            rq.pop();
                        }
                    }
                }
    ...
    for (int i = 0; i < thread_count; i++) {
        graph->merge_nodes(graph_threads[i]->nodes());
        graph->merge_edges(graph_threads[i]->edges());
    }
