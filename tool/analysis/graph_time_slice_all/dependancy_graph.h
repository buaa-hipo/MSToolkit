...
struct RecordNode {
    ...
    RecordNode(int rank, int ts_id, bool enclosed, record_t* r, uint64_t offset)
        : rank(rank),
          ts_id(ts_id),
          enclosed(enclosed),
          data(static_cast<record_t*>(malloc(record_utils::get_record_size(r)))), 
    ...
    RecordNode(int rank, int ts_id, bool enclosed, record_t* r, uint64_t offset, const std::vector<std::pair<uint64_t, uint64_t>>& pmu_values)
        : rank(rank),
          ts_id(ts_id),
          enclosed(enclosed),
          data(static_cast<record_t*>(malloc(record_utils::get_record_size(r)))), 
    ...
    RecordNode(int rank, int ts_id, bool enclosed, record_t* r, uint64_t offset, uint64_t key)
        : rank(rank),
          ts_id(ts_id),
          enclosed(enclosed),
          data(static_cast<record_t*>(malloc(record_utils::get_record_size(r)))), 
    ...
    // RecordNode(int rank, int ts_id, bool enclosed, record_t* r, uint64_t offset, uint64_t key) : rank(rank), ts_id(ts_id), enclosed(enclosed), data(r), offset(offset), key(key) {}
...
public:
    ...
    void dump_nodes(const char* filename, const std::vector<std::string>& event_list, RecordTraceExtCollection& rte_collection);

    void dump_backtrace(const char* filename, BacktraceCollection* backtrace, const char* input_file);
...
DependancyGraph* CreateDependancyGraphSliceAll(RecordTraceCollection& collection, RecordTraceExtCollection& rte_collection, RankMetaCollection& metas, bool need_serial_link, uint64_t interval, uint64_t duration);
