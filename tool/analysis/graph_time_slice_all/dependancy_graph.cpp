...
DependancyGraph* CreateDependancyGraphSliceAll(RecordTraceCollection& collection, RecordTraceExtCollection& rte_collection, RankMetaCollection& metas, bool need_serial_link, uint64_t interval, uint64_t duration){
    ...
    for(uint64_t ts = start_time; ts < exit_time; ts += interval){;
            ...
            for(auto ri=rtrace.begin(), re=rtrace.end(); ri!=re; ri=ri.next()) {
                ...
                for(RecordTraceExtIterator rit = rte->begin(); rit.valid() && !matched; rit.next()) {
                    ...
                    if (era->correlation_id == cid) {
                        matched = true;
                        // device record_t
                        record_t* er = new record_t();
                        auto mhz = get_tsc_freq_mhz();
                        er->timestamps.enter = ns_to_tsc(era->begin_ns, mhz);
                        er->timestamps.exit = ns_to_tsc(era->end_ns, mhz);

                        r_node =  new RecordNode(-rank-1, ts_id, true, er, offset, era->sym_key.key); //TODO !!!!!!!!
                        r_node->event_id = event_id++;
                        graph->add_node(-rank-1, r_node);
                        graph->add_edge(new DependancyEdge(node, r_node,  DependencyType::HIP));
                        graph->add_edge(new DependancyEdge(r_node, node,  DependencyType::HIP));
                        break;
                    }
                }
                if (!matched) {
        ...
        /* generate device-host dependancy linkages */
...
void DependancyGraph::dump_nodes(const char* filename, const std::vector<std::string>& event_list,  RecordTraceExtCollection& rte_collection) {
    auto fp = fopen(filename, "w");
    if (!fp) {
        perror("Failed to open file");
        return;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if(rank >= 0) {
            ...
            record_t* r = _nodes[i]->data;
            name = demangle(rte_collection[-rank-1]->get_string(_nodes[i]->key)) ; 
            double start_time = tsc_duration_us(r->timestamps.enter - off, mhz);
            double duration = tsc_duration_us(r->timestamps.exit - off, mhz) - start_time;
            ts = std::to_string(start_time);
            dur = std::to_string(duration);
