...
#include <unistd.h>
#include "master.h"
#define PRINT(__STR__) if(mpi_rank==0) printf("\n%s\n",__STR__);
#define N_spinor_buf 26
#define N_su3_buf 1
double dslash_send = 0.0, dslash_main = 0.0, dslash_recv = 0.0, dslash_recv_acc = 0.0, re1 = 0.0,re2 = 0.0,mpi_time = 0.0,allre1 = 0.0,allre2=0.0,dslash_all=0.0;
int cluster_id;
int group_id;
void* global_buf;
...
void mtransfer_conf_init(){
	mtransfer->link_x = (su3_sliceX *)hthread_malloc(cluster_id, sizeof(FLOAT) * NY * NT * NZ * 2 * 3 * 2+N_align, HT_MEM_RW);
			...
			for (int a = 0; a < 2; ++a)
				for (int b = 0; b < 3; ++b)
					for (int c = 0; c < 2; ++c)
						for (int k = 0; k < NY; ++k)
						{
							ptr[0][i][j][k/16][a][b][c][k%16] = global_link_buf[0][0][i][j][k][NX/16-1][a][b][c][15];
						}
	...
	// 					        *p1 = (it*NZ*NY*NX*3*4*2 + iz*NY*NX*3*4*2 + (iy*16+iiy)*3*4*2*NX+a*4*2*16+b*2*16+c*16)*8;
    //                             //printf("%d ",*p1);
    //                             ++p1;
    //                             *p2 = (it*NZ*NY*NX*3*4*2 + iz*NY*NX*3*4*2 + (iy*16+iiy)*3*4*2*NX+a*4*2*16+b*2*16+c*16+3*4*2*16+15)*8;
    //                             ++p2;
	// 				        }
        //return;
    //}
...
void init(){
    ...
    reduce_init();
    PRINT("data init.....");
    global_buf = hthread_malloc(cluster_id,N_spinor_buf*sizeof(spinor_field)+N_su3_buf*sizeof(su3_field)+N_align,HT_MEM_RW);
    if(global_buf==NULL) {
    ...
    mtransfer_conf_init();
    if(hthread_dat_load(cluster_id,"dsp_kernel.dat")!=HT_SUCCESS){
        PRINT("Failed to load dat file!");
        return;
    }
    group_id = hthread_group_create(cluster_id,NThreads,NULL,NULL,NULL,NULL);
    if(group_id<0){
...
int main(int argc, char** argv){
    try{
        ...
        for(int i = 0; i < 4; ++i){
            ...
            mr_solver(global_spinor_buf[i], global_link_buf[0], global_spinor_buf[i+2], global_spinor_buf[i+1], global_spinor_buf[i+14]);
    ...
    hthread_free(global_buf);
    hthread_free(mtransfer);
    hthread_free(reduce_buffer);
