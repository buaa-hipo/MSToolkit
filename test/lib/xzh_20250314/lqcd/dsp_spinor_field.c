...
void cal_sum(Para* local_para){
	...
	/*if(local_para->thread_id==0){
		hthread_printf("residual:%lf\n",local_para->residual_square);
		hthread_printf("\n");
		hthread_printf("#1:%lf\n",vsip_vsumval_d_v(rebuf,NX));
		hthread_printf("#2:%lf\n",vsumval_double(*rebuf));
	}*/
...
void spinor_field_acc_linear_combine_real(spinor_field dst, spinor_field src, double coef_dst, double coef_s, Para* local_para){
    ...
    for(i=0;i<NT;++i){
                ...
                for(int ia = 0; ia < 3; ++ia)
                    for(int ib = 0; ib < 4; ++ib)
                        for(int ir = 0; ir < 2; ++ir){
                            ...
                            //if(i==0&&iy==0&&ia==0&&ib==0&&ir==0&&local_para->thread_id==0) hthread_printf("%d\n",gsm_mem[i*NZ*NY*24+(local_para->thread_id)*NY*24+(iy>>4)*24*16+ia*8*16+ib*2*16+ir*16+(iy&15)]);
                            //if(i==0&&iy==0&&ia==0&&ib==0&&ir==0&&local_para->thread_id==0) hthread_printf("%lf\n",gsm_mem[0][i][local_para->thread_id][iy>>4][ia][ib][ir][iy&15]);
                            mov_to_vlr(0x0001);
        ...
        cal_sum(local_para);
        dma_wait(dma_handle);
        //if(local_para->thread_id==0) hthread_printf("#%d:%lf\n",i,local_para->residual_square);
    }
}

void print_lvector_double(lvector double lv){
    mov_to_svr_v16df(lv);
    hthread_printf("%lf ",mov_from_svr0_df());
}
