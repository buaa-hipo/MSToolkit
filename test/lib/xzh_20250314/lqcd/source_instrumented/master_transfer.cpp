...
void transfer_init(){
    ...
    for(i=0;i<2;++i)
        for(j=0;j<4;++j){
			...
			Dsend[i][j].buf=(FLOAT*)instrumented_hthread_malloc(cluster_id,data_len[j]*sizeof(FLOAT)+N_align,HT_MEM_RW);
			Dsend[i][j].buf = (FLOAT*)align_f4((char*)Dsend[i][j].buf);
			mtransfer->data_send_recv[0][i][j].ptr = Dsend[i][j].buf;
			Drecv[i][j].buf=(FLOAT*)instrumented_hthread_malloc(cluster_id,data_len[j]*sizeof(FLOAT)+N_align,HT_MEM_RW);
...
void mpi_init(){
	torus_comm_init();
	if(instrumented_hthread_dev_open(cluster_id)!=HT_SUCCESS){ //未启动设备无法申请内存
        char name[256];
		int len = 256;
	...
	mtransfer = (master_transfer*)instrumented_hthread_malloc(cluster_id,sizeof(master_transfer),HT_MEM_RW);
