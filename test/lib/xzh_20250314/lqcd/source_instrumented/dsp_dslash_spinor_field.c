...
void dslash(spinor_field dst, su3_field link, spinor_field src, Para* local_para){
    int i,j,k;
    int dma_handle;
    dma_pull(local_para->local_ferm[0], src [16 -1][local_para->thread_id][0],sizeof(spinor_local),1,0);
    spinor_local_zero(local_para->local_ferm[1]);
    for(i=16 -1; i>0; --i){



        dma_pull(local_para->local_link,link[2] [i][local_para->thread_id][0], sizeof(su3_local),1,0);
        ...
        dma_pull(local_para->local_link, link[1] [i][local_para->thread_id][0], sizeof(su3_local), 1, 0);
        dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0],2,e_forward,
# 26 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                           ((void *)0)
# 26 "dsp_dslash_spinor_field.c"
                                                                                                                               );
        dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0],2,e_backward,
# 27 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                            ((void *)0)
# 27 "dsp_dslash_spinor_field.c"
                                                                                                                                );


        dma_pull(local_para->local_link, link[0] [i][local_para->thread_id][0], sizeof(su3_local), 1, 0);
        ...
        dma_pull(local_para->local_link, link[3] [i-1][local_para->thread_id][0], sizeof(su3_local), 1, 0);
        dslash_spinor_local(local_para->local_ferm[2], local_para->local_link[0], local_para->local_ferm[0], 0, e_forward,
# 38 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                         ((void *)0)
# 38 "dsp_dslash_spinor_field.c"
                                                                                                                             );

        dma_pull(local_para->local_ferm[0], src [i-1][local_para->thread_id][0], sizeof(spinor_local),1,0);
        dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 0, e_backward,
# 41 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                              ((void *)0)
# 41 "dsp_dslash_spinor_field.c"
                                                                                                                                  );

                              dma_push(local_para->local_ferm[1], dst [i][local_para->thread_id][0], sizeof(spinor_local),1,0);
    ...
    dma_pull(local_para->local_link, link[2] [0][local_para->thread_id][0],sizeof(su3_local), 1, 0);
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0],local_para->local_ferm[0], 1, e_forward,local_para->local_ferm[2]);
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 1, e_backward,local_para->local_ferm[2]);

    dma_pull(local_para->local_link, link[1] [0][local_para->thread_id][0], sizeof(su3_local), 1, 0);
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 2, e_forward,
# 58 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                         ((void *)0)
# 58 "dsp_dslash_spinor_field.c"
                                                                                                                             );
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 2, e_backward,
# 59 "dsp_dslash_spinor_field.c" 3 4
                                                                                                                          ((void *)0)
# 59 "dsp_dslash_spinor_field.c"
                                                                                                                              );

    dma_pull(local_para->local_link, link[0] [0][local_para->thread_id][0], sizeof(su3_local), 1, 0);
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 3, e_forward,local_para->local_ferm[2]);
    dslash_spinor_local_acc(local_para->local_ferm[1], local_para->local_link[0], local_para->local_ferm[0], 3, e_backward,local_para->local_ferm[2]);

    dma_push(local_para->local_ferm[1], dst [0][local_para->thread_id][0], sizeof(spinor_local), 1, 0);
}
void push_neighbour(t_dir dir,Para* local_para, int dim){
    void* dst = 
# 68 "dsp_dslash_spinor_field.c" 3 4
               ((void *)0)
# 68 "dsp_dslash_spinor_field.c"
                   ;
    switch (dim)
    {
    case 0:
...
int pull_neighbour_async(spinor_local dst,Para* local_para, int dim,t_dir dir){
    void* src = 
# 92 "dsp_dslash_spinor_field.c" 3 4
               ((void *)0)
# 92 "dsp_dslash_spinor_field.c"
                   ;
    int dma_handle;
    switch (dim)
    {
...
void push_forward_to_cpu(Para* local_para, int dim){
    void* dst = 
# 118 "dsp_dslash_spinor_field.c" 3 4
               ((void *)0)
# 118 "dsp_dslash_spinor_field.c"
                   ;
    switch (dim)
    {
    case 0:
...
# 161 "dsp_dslash_spinor_field.c"
void dma32(void* dst, void* src,t_dir dir){
    double* ddst = (double*) dst;
    double* dsrc = (double*) src + (dir == e_forward? 0 : 15);
    for(int iz = 0; iz < 16; ++iz){
        for(int i = 0; i < 24; ++i)
            dma_pull(ddst+24*32*iz+16*i,dsrc+24*32*32*iz+16*i,sizeof(double),16,32/16*sizeof(spinor_vec)-sizeof(double));

        for(int i = 0; i < 24; ++i)
            dma_pull(ddst+24*32*iz+16*24+16*i,dsrc+24*32*32*iz+16*24*32+16*i,sizeof(double),16,32/16*sizeof(spinor_vec)-sizeof(double));
    }
...
void dslash_spinor_backward_neighbour_field(su3_field link, spinor_field src,Para* local_para,spinor_sliceX backward_spinor,spinor_sliceX forward_spinor){
    dma_pull(local_para->local_ferm[1],src [0][local_para->thread_id][0], sizeof(spinor_local),1,0);
    push_neighbour(e_forward,local_para,0);

    dma_pull(local_para->local_ferm[1],src [local_para->thread_id][0][0], sizeof(spinor_local),1,0);
    push_neighbour(e_forward,local_para,1);

    dma_pull(local_para->local_ferm[1],src [local_para->thread_id][0][0],sizeof(spinor_vec)*32/16,16,(32 -1)*32/16*sizeof(spinor_vec));
    push_neighbour(e_forward,local_para,2);

    double* gsm_mem = dsp_comm;


    dma_pull(local_para->local_ferm[1],gsm_mem+(16 +(local_para->thread_id))*16*32*24,sizeof(spinor_local)/2,1,0);
    push_neighbour(e_forward,local_para,3);


    dma_pull(local_para->local_link, local_para->mtransfer->link_x[0] [local_para->thread_id][0][0][0/16],sizeof(su3_local)/2,1,0);



    dma_pull(local_para->local_ferm[0],gsm_mem+(local_para->thread_id)*16*32*24,sizeof(spinor_local)/2,1,0);
    int dma_handle1 = dma_pull_async(local_para->local_ferm[2],src [local_para->thread_id][0][32 -1],sizeof(spinor_vec)*32/16,16,(32 -1)*32/16*sizeof(spinor_vec));
    ...
    dma_pull(local_para->local_link,link[1] [local_para->thread_id][0][32 -1],sizeof(su3_vec)*32/16,16,(32 -1)*32/16*sizeof(su3_vec));
    instrumented_dma_wait(dma_handle1);
    dma_handle1 = dma_pull_async(local_para->local_ferm[0], src [local_para->thread_id][16 -1][0],sizeof(spinor_local),1,0);
    ...
    dma_pull(local_para->local_link,link[2] [local_para->thread_id][16 -1][0],sizeof(su3_local),1,0);
    instrumented_dma_wait(dma_handle1);
    dma_handle1 = dma_pull_async(local_para->local_ferm[2], src [16 -1][local_para->thread_id][0], sizeof(spinor_local), 1, 0);
    ...
    dma_pull(local_para->local_link, link[3] [16 -1][local_para->thread_id][0], sizeof(su3_local), 1, 0);
    instrumented_dma_wait(dma_handle1);
...
void push_to_data_x(spinor_field dst,Para* local_para){
    typedef double sf_[16][32][32/16][3][4][2][16];
    ...
    for(i=0;i<16;++i)
        for(j=0;j<2;++j)
            for(a=0;a<3;++a)
                for(b=0;b<4;++b)
                    for(c=0;c<2;++c){
                        __builtin_mov_to_svr_v16df(local_para->local_ferm[1][i][j][a][b][c]);
                        sf[0][i][j*16+0][0][a][b][c][0] += __builtin_mov_from_svr0_df();
                        sf[0][i][j*16+1][0][a][b][c][0] += __builtin_mov_from_svr1_df();
                        sf[0][i][j*16+2][0][a][b][c][0] += __builtin_mov_from_svr2_df();
                        sf[0][i][j*16+3][0][a][b][c][0] += __builtin_mov_from_svr3_df();
                        sf[0][i][j*16+4][0][a][b][c][0] += __builtin_mov_from_svr4_df();
                        sf[0][i][j*16+5][0][a][b][c][0] += __builtin_mov_from_svr5_df();
                        sf[0][i][j*16+6][0][a][b][c][0] += __builtin_mov_from_svr6_df();
                        sf[0][i][j*16+7][0][a][b][c][0] += __builtin_mov_from_svr7_df();
                        sf[0][i][j*16+8][0][a][b][c][0] += __builtin_mov_from_svr8_df();
                        sf[0][i][j*16+9][0][a][b][c][0] += __builtin_mov_from_svr9_df();
                        sf[0][i][j*16+10][0][a][b][c][0] += __builtin_mov_from_svr10_df();
                        sf[0][i][j*16+11][0][a][b][c][0] += __builtin_mov_from_svr11_df();
                        sf[0][i][j*16+12][0][a][b][c][0] += __builtin_mov_from_svr12_df();
                        sf[0][i][j*16+13][0][a][b][c][0] += __builtin_mov_from_svr13_df();
                        sf[0][i][j*16+14][0][a][b][c][0] += __builtin_mov_from_svr14_df();
                        sf[0][i][j*16+15][0][a][b][c][0] += __builtin_mov_from_svr15_df();
                    }
}
void dslash_spinor_forward_neighbour_field(spinor_field dst, su3_field link, spinor_field src,Para* local_para){
    dma_pull(local_para->local_link, local_para->mtransfer->link_x[0] [local_para->thread_id][0][0][0/16],sizeof(su3_local)/2,1,0);
    ...
    instrumented_dma_wait(dma_handle2);
    ...
    dma_push(local_para->local_ferm[1],gsm_mem+(16 +(local_para->thread_id))*16*32*24,sizeof(spinor_local)/2,1,0);

    dma_pull(local_para->local_link,link[1] [local_para->thread_id][0][32 -1],sizeof(su3_vec)*32/16,16,(32 -1)*32/16*sizeof(su3_vec));
    dma_pull(local_para->local_ferm[1],dst [local_para->thread_id][0][32 -1],32/16*sizeof(spinor_vec),16,(32 -1)*32/16*sizeof(spinor_vec));
    instrumented_dma_wait(dma_handle1);
    ...
    dma_push(local_para->local_ferm[1],dst [local_para->thread_id][0][32 -1],32/16*sizeof(spinor_vec),16,(32 -1)*32/16*sizeof(spinor_vec));


    dma_pull(local_para->local_link,link[2] [local_para->thread_id][16 -1][0],sizeof(su3_local),1,0);
    dma_pull(local_para->local_ferm[1],dst [local_para->thread_id][16 -1][0],sizeof(spinor_local),1,0);
    instrumented_dma_wait(dma_handle1);
    ...
    dma_push(local_para->local_ferm[1],dst [local_para->thread_id][16 -1][0],sizeof(spinor_local),1,0);


    dma_pull(local_para->local_link, link[3] [16 -1][local_para->thread_id][0], sizeof(su3_local), 1, 0);
    dma_pull(local_para->local_ferm[1],dst [16 -1][local_para->thread_id][0],sizeof(spinor_local),1,0);
    instrumented_dma_wait(dma_handle1);
    ...
    dma_push(local_para->local_ferm[1],dst [16 -1][local_para->thread_id][0],sizeof(spinor_local),1,0);


    dma_pull(local_para->local_ferm[1],dst [0][local_para->thread_id][0],sizeof(spinor_local),1,0);
    instrumented_dma_wait(dma_handle1);
    ...
    dma_pull(local_para->local_ferm[1], dst [local_para->thread_id][0][0],sizeof(spinor_local),1,0);
    instrumented_dma_wait(dma_handle1);
    ...
    dma_push(local_para->local_ferm[1],dst [local_para->thread_id][0][0],sizeof(spinor_local),1,0);


    dma_pull(local_para->local_ferm[1],dst [local_para->thread_id][0][0],32/16*sizeof(spinor_vec),16,(32 -1)*32/16*sizeof(spinor_vec));
    instrumented_dma_wait(dma_handle1);
    dma_handle1 = pull_neighbour_async(local_para->local_ferm[2],local_para,3,e_backward);
    spinor_local_acc(local_para->local_ferm[1],local_para->local_ferm[0]);
    dma_push(local_para->local_ferm[1],dst [local_para->thread_id][0][0],32/16*sizeof(spinor_vec),16,(32 -1)*32/16*sizeof(spinor_vec));


    instrumented_dma_wait(dma_handle1);
    dma_push(local_para->local_ferm[2],gsm_mem+(local_para->thread_id)*16*32*24,sizeof(spinor_local)/2,1,0);
}
