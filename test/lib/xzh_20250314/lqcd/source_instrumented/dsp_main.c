...
__attribute__ ((section (".gsm"))) DSP_COMM dsp_comm[16][2];
volatile int dsp_comm_flag[16][2];
...
void dsp_para_init(Para* p,master_transfer* mtransfer){
    ...
    p->local_link = instrumented_vector_malloc(sizeof(su3_local));
    p->local_ferm = instrumented_vector_malloc(sizeof(spinor_local)*3);
    p->zero_vec = instrumented_vector_malloc(sizeof(__attribute__((vector_size (128))) double));
    p->zero_vec[0] = 0.0;
    p->mtransfer = mtransfer;
}
void dsp_para_free(Para* p){
    instrumented_vector_free(p->local_link);
    instrumented_vector_free(p->local_ferm);
    instrumented_vector_free(p->zero_vec);
}

__attribute__ ((section (".global"))) void init_gsm(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, spinor_sliceX backward_spinor,spinor_sliceX forward_spinor){
instrumented_kernel("init_gsm", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    dsp_para_init(&local_para,
# 28 "dsp_main.c" 3 4
                             ((void *)0)
# 28 "dsp_main.c"
                                 );
    double* d = (double*) dsp_comm;
    dma_pull(local_para.local_ferm[0],backward_spinor[local_para.thread_id],sizeof(spinor_sliceX)/16,1,0);
    dma_push(local_para.local_ferm[0],d+(local_para.thread_id)*16*32*24,sizeof(spinor_sliceX)/16,1,0);

    dma_pull(local_para.local_ferm[0],forward_spinor[local_para.thread_id],sizeof(spinor_sliceX)/16,1,0);
    dma_push(local_para.local_ferm[0],d+(16 +local_para.thread_id)*16*32*24,sizeof(spinor_sliceX)/16,1,0);

    dsp_para_free(&local_para);
instrumented_kernel("init_gsm", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}

__attribute__ ((section (".global"))) void dslash_backward_neighbour(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, su3_field link, spinor_field src, master_transfer* mtransfer,spinor_sliceX backward_spinor,spinor_sliceX forward_spinor){
instrumented_kernel("dslash_backward_neighbour", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    ...
    dsp_para_free(&local_para);
instrumented_kernel("dslash_backward_neighbour", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}

__attribute__ ((section (".global"))) void dslash_main(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, spinor_field dst, su3_field link, spinor_field src){
instrumented_kernel("dslash_main", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    dsp_para_init(&local_para,
# 48 "dsp_main.c" 3 4
                             ((void *)0)
# 48 "dsp_main.c"
    ...
    dsp_para_free(&local_para);
instrumented_kernel("dslash_main", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}

__attribute__ ((section (".global"))) void dslash_forward_neighbour(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, spinor_field dst, su3_field link, spinor_field src, master_transfer* mtransfer){
instrumented_kernel("dslash_forward_neighbour", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    dsp_para_init(&local_para,mtransfer);
    dslash_spinor_forward_neighbour_field(dst,link,src,&local_para);
    dsp_para_free(&local_para);
instrumented_kernel("dslash_forward_neighbour", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}

__attribute__ ((section (".global"))) void dsp_reduce1(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, spinor_field dst, spinor_field src, double* reduce_buffer){
instrumented_kernel("dsp_reduce1", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    dsp_para_init(&local_para,
# 64 "dsp_main.c" 3 4
                             ((void *)0)
# 64 "dsp_main.c"
    ...
    spinor_field_acc_linear_combine_real(dst,src,-0.5,1.0/(2*0.135),&local_para);
    ...
    dsp_para_free(&local_para);
instrumented_kernel("dsp_reduce1", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}

__attribute__ ((section (".global"))) void dsp_reduce2(unsigned int host_pid, unsigned long long kernel_cid, unsigned int kernel_index, spinor_field dst, spinor_field src, spinor_field aux,double *coef_, double* reduce_buffer,double* backward_spinor, double* forward_spinor){
instrumented_kernel("dsp_reduce2", host_pid, kernel_cid, kernel_index, ACCL_API_ENTER);
    Para local_para;
    dsp_para_init(&local_para,
# 78 "dsp_main.c" 3 4
                             ((void *)0)
# 78 "dsp_main.c"
    ...
    spinor_field_acc_complex_mul(dst,coef,src,&local_para,
# 85 "dsp_main.c" 3 4
                                                         ((void *)0)
# 85 "dsp_main.c"
                                                             ,
...
# 85 "dsp_main.c"
    ...
    dsp_para_free(&local_para);
instrumented_kernel("dsp_reduce2", host_pid, kernel_cid, kernel_index, ACCL_API_EXIT);
}
