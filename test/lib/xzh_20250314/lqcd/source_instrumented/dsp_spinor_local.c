...
# 5 "dsp_spinor_local.c" 2

extern volatile int dsp_comm_flag[16][2];
...
void spinor_local_shift(spinor_local d, spinor_local s, int dim, t_dir dir){
    ...
    if(dim==1){
        if(dir==e_forward){
            if(thread_id!=0){
                while(!dsp_comm_flag[thread_id][e_forward]);
                dma_push(s,dsp_comm[thread_id][e_forward].data,sizeof(spinor_local),1,0);
                dsp_comm_flag[thread_id][e_forward] = 0;
            }
            if(thread_id==16 -1){
                spinor_local_zero(d);
            }else{
                while(dsp_comm_flag[thread_id+1][e_forward]);
                dma_pull(d,dsp_comm[thread_id+1][e_forward].data,sizeof(spinor_local),1,0);
        ...
        if(dir==e_forward){
            for(i=0;i<32;++i){
                for(j=0;j<32/16-1;++j)
                    for(k=0;k<3;++k)
                        for(l=0;l<4;++l){
                            d[i][j][k][l][0] = __builtin_vec_ld(1,&s[i][j][k][l][0]);
                            d[i][j][k][l][1] = __builtin_vec_ld(1,&s[i][j][k][l][1]);
                            __builtin_mov_to_vlr(0x8000);
                            d[i][j][k][l][0] = __builtin_vec_ld(1,&s[i][j+1][k][l][0]-1);
                            d[i][j][k][l][1] = __builtin_vec_ld(1,&s[i][j+1][k][l][1]-1);
                ...
                for(k=0;k<3;++k)
                    for(l=0;l<4;++l){
                        ...
                        d[i][j][k][l][1] = __builtin_vec_ld(1,&s[i][j][k][l][1]);
                        __builtin_mov_to_vlr(0x8000);
                        __builtin_vec_st(__builtin_vec_movi(0.0),0,&d[i][j][k][l][0]);
                        __builtin_vec_st(__builtin_vec_movi(0.0),0,&d[i][j][k][l][1]);
                        __builtin_mov_to_vlr(0xFFFF);
                    }
            }
            ...
            for(i=0;i<32;++i){
                for(j=32/16-1;j>0;--j){
                    for(k=0;k<3;++k)
                        for(l=0;l<4;++l){
                            d[i][j][k][l][0] = __builtin_vec_ld(15,&s[i][j][k][l][0]-1);
                            d[i][j][k][l][1] = __builtin_vec_ld(15,&s[i][j][k][l][1]-1);

                            __builtin_mov_to_vlr(0x0001);
                            d[i][j][k][l][0] = __builtin_vec_ld(15,&s[i][j-1][k][l][0]);
                            d[i][j][k][l][1] = __builtin_vec_ld(15,&s[i][j-1][k][l][1]);
                            __builtin_mov_to_vlr(0xFFFF);
