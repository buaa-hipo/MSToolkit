...
void reduce1(spinor_field dst,spinor_field src){
    ...
    hthread_group_exec(group_id,"dsp_reduce1",0,3,args);
...
void reduce2(double* coef, spinor_field dst, spinor_field src, spinor_field aux){
    ...
    auto start = std::chrono::high_resolution_clock::now();
    hthread_group_exec(group_id,"dsp_reduce2",0,7,args);
    hthread_group_wait(group_id);
    auto end = std::chrono::high_resolution_clock::now();
...
void reduce_init(){
    reduce_buffer = (double*) hthread_malloc(cluster_id,NThreads*3*sizeof(double),HT_MEM_RW);
    //
    dslash_backward_spinor = (spinor_sliceX*) hthread_malloc(cluster_id,sizeof(spinor_sliceX),HT_MEM_RW);
    dslash_forward_spinor = (spinor_sliceX*) hthread_malloc(cluster_id,sizeof(spinor_sliceX),HT_MEM_RW);
}
