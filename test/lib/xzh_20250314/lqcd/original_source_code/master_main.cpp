...
void mtransfer_conf_init() {
    mtransfer->link_x =
        (su3_sliceX*)hthread_malloc(cluster_id, sizeof(FLOAT) * NY * NT * NZ * 2 * 3 * 2 + N_align, HT_MEM_RW);
...
void init() {
    ...
    group_id = hthread_group_create(cluster_id, NThreads);
    if (group_id < 0) {
