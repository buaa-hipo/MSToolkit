      ...
      for(int iy = 0; iy < 4;++iy)
        for(int ix = 0; ix < NX; ++ix)
          for(int a = 0; a < 3; ++a)
            for(int b = 0; b < 4; ++b)
              for(int r = 0; r < 2; ++r){
                ret+=s[it][iz][iy][ix>>4][a][b][r][ix&15]*s[it][iz][iy][ix>>4][a][b][r][ix&15];
              }
    ...
    for(int iz = 0; iz < NZ; ++iz)
      for(int a = 0; a < 3; ++a)
        for(int b = 0; b < 4; ++b)
          for(int r = 0; r < 2; ++r)
            for(int iy = 0; iy < NY; ++iy){
              dslash_forward_spinor[0][it][iz][iy>>4][a][b][r][iy&15] = src[it][iz][iy][0][a][b][r][0];
              dslash_backward_spinor[0][it][iz][iy>>4][a][b][r][iy&15] = src[it][iz][iy][1][a][b][r][15];
            }
  uint64_t args[2];
  args[0] = (uint64_t) dslash_backward_spinor;
  args[1] = (uint64_t) dslash_forward_spinor;
  hthread_group_exec(group_id,"init_gsm",0,2,args);
...
void mr_solver(spinor_field dst, su3_field link, spinor_field src, spinor_field aux, spinor_field src0){
    double *coef = (double*)hthread_malloc(cluster_id,2*sizeof(double),HT_MEM_RW), residual;
