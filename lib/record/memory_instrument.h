...
#define MEMORY_WRAPPER_INIT_PRIORITY (RECORD_INIT_PRIORITY-10)
#define MEMORY_WRAPPER_FINI_PRIORITY (RECORD_FINI_PRIORITY-10)
...
std::atomic<int> collect_memory_info_flag(0);

bool enable_memory_collect = false;
...
static void *(*real_memalign)(size_t, size_t) = (void*(*)(size_t,size_t))0x6;
static void *(*real_aligned_alloc)(size_t, size_t) = (void*(*)(size_t,size_t))0x7;
static int (*real_posix_memalign)(void **,size_t,size_t) = (int(*)(void**,size_t,size_t))0x8;
...
void jsi_memory_tracer_init(){
    char* env = getenv("JSI_ENABLE_MEMORY_COLLECT");
    if (env && std::strcmp(env, "ON") == 0) {
        enable_memory_collect = true;
    }
...
record_memory_malloc* jsi_enter_malloc(record_memory_malloc* rec, uint16_t MsgType) {
    if(collect_memory_info_flag.load() == 0)
    {
        collect_memory_info_flag.store(1);
        collect_system_info();
    }
...
inline __attribute__((always_inline))
record_memory_memalign* jsi_enter_memalign(record_memory_memalign* rec, uint16_t MsgType) {
    allocate_time += 1;
    rec[0].record.MsgType = (int16_t)MsgType;
    rec[0].record.timestamps.enter = get_tsc_raw();
#ifdef ENABLE_BACKTRACE
...
inline __attribute__((always_inline))
void jsi_exit_memalign(record_memory_memalign* rec, const void* ptr, size_t alignment, size_t size_bytes) {
    record_memory_memalign& r = reinterpret_cast<record_memory_memalign*>(rec)[0];
    r.record.timestamps.exit = get_tsc_raw();
    r.ptr = (const void*) ptr;
...
#ifdef ENABLE_PMU
    if (jsi_pmu_enabled) {
        uint64_t* counters = RecordWriterHelper::counters(reinterpret_cast<record_memory_memalign*>(rec));
        pmu_collector_get_all(counters + jsi_pmu_num);
    }
#endif
    RecordWriter::traceStore((const record_t*)rec);
    DEALLOCATE(rec, sizeof(record_memory_memalign) + sizeof(uint64_t) * 2 * jsi_pmu_num);
}

inline __attribute__((always_inline))
record_memory_aligned_alloc* jsi_enter_aligned_alloc(record_memory_aligned_alloc* rec, uint16_t MsgType) {
    allocate_time += 1;
    rec[0].record.MsgType = (int16_t)MsgType;
    rec[0].record.timestamps.enter = get_tsc_raw();
#ifdef ENABLE_BACKTRACE
...
inline __attribute__((always_inline))
void jsi_exit_aligned_alloc(record_memory_aligned_alloc* rec, const void* ptr, size_t alignment, size_t size_bytes) {
    record_memory_aligned_alloc& r = reinterpret_cast<record_memory_aligned_alloc*>(rec)[0];
    r.record.timestamps.exit = get_tsc_raw();
    r.ptr = (const void*) ptr;
    r.alignment = alignment;
    r.size_bytes = size_bytes;
#ifdef ENABLE_PMU
    if (jsi_pmu_enabled) {
        uint64_t* counters = RecordWriterHelper::counters(reinterpret_cast<record_memory_aligned_alloc*>(rec));
        pmu_collector_get_all(counters + jsi_pmu_num);
    }
#endif
    RecordWriter::traceStore((const record_t*)rec);
    DEALLOCATE(rec, sizeof(record_memory_aligned_alloc) + sizeof(uint64_t) * 2 * jsi_pmu_num);
}

inline __attribute__((always_inline))
record_memory_posix_memalign* jsi_enter_posix_memalign(record_memory_posix_memalign* rec, uint16_t MsgType) {
    allocate_time += 1;
    rec[0].record.MsgType = (int16_t)MsgType;
    rec[0].record.timestamps.enter = get_tsc_raw();
#ifdef ENABLE_BACKTRACE
...
inline __attribute__((always_inline))
void jsi_exit_posix_memalign(record_memory_posix_memalign* rec, const void* ptr, size_t alignment, size_t size_bytes, int error_code) {
    record_memory_posix_memalign& r = reinterpret_cast<record_memory_posix_memalign*>(rec)[0];
    r.record.timestamps.exit = get_tsc_raw();
    r.ptr = (const void*) ptr;
    r.alignment = alignment;
    r.size_bytes = size_bytes;
...
#ifdef ENABLE_PMU
    if (jsi_pmu_enabled) {
        uint64_t* counters = RecordWriterHelper::counters(reinterpret_cast<record_memory_posix_memalign*>(rec));
        pmu_collector_get_all(counters + jsi_pmu_num);
    }
#endif
    RecordWriter::traceStore((const record_t*)rec);
    DEALLOCATE(rec, sizeof(record_memory_posix_memalign) + sizeof(uint64_t) * 2 * jsi_pmu_num);
}
