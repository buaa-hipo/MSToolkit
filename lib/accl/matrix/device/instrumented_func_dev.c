...
void instrumented_kernel(const char *name, uint32_t host_pid, uint64_t kernel_cid, uint32_t kid,
...
/// ! Modified so that activity record for kernel can be in the same trace file as callback records.
...
#else
    matrix_api_data_t *record = (matrix_api_data_t *)malloc(sizeof(matrix_api_data_t));
    record->kernel.domain     = ACCL_DOMAIN_HTHREAD_OPS;
    record->kernel.kind       = ACCL_ACTIVITY_kernel;
    record->kernel.op         = kid;
    record->correlation_id    = kernel_cid;
    record->kernel.process_id = host_pid;
    record->kernel.thread_id  = get_core_id();
    record->phase             = ACCL_API_ENTER;
    if (phase == ACCL_API_ENTER) {
        correlation_id[get_core_id()] = kernel_cid + 1;
        MT_PMU_collector_init();
        dev_api_callback(ACCL_DOMAIN_HTHREAD_OPS, kid, record, NULL);
        return;
    }
    record->phase = ACCL_API_EXIT;
    dev_api_callback(ACCL_DOMAIN_HTHREAD_OPS, kid, record, NULL);
#endif
    ...
    hthread_printf("\tProcess ID: %d\n", record->process_id);
    hthread_printf("\tThread ID: %d\n", record->thread_id);
    hthread_printf("\tBegin TS: %llu\n", record->begin_ns);
    hthread_printf("\tEnd TS: %llu\n", record->end_ns);
    hthread_printf("\tOP-kid: %u, %u\n", record->op, kid);
    hthread_printf("}\n");
#endif

    MT_PMU_collector_fin();
}

void instrumented_func(void *func, const char *func_name, unsigned long cid, accl_api_phase_t phase) {
...
static accl_memcpy_kind_t ACCL_get_memcpy_direction(void *src, void *dst) {
    if (src >= DDR_BASE) {  // from off
        ...
        JSI_ERROR("Memcpy between on-core memory is not allowed\n");
    }
}
